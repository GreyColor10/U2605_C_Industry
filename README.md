...
- Actor 스폰 방식은 객체 수에 비례해 비용이 증가하지만, 파티클 배열 갱신 방식은 객체 수가 늘어도 드로우콜이 일정하게 유지되는 구조 (확장성 확보)
...
- 창고의 자동 출고 주기 조정으로 병목현상을 유발할 수 있습니다.
...
| 정적 설비 메시를 **HISM으로 통합 관리** (`UCInstancedMeshSubsystem`) | 동일 메시를 사용하는 설비를 인스턴스 하나로 묶어 드로우콜 최소화. `SetCustomDataValue`로 설비별 가동 상태(발광)를 머티리얼에 전달 |
